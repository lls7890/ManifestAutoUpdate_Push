...
def get_manifest(cdn, app_id, appinfo, package, manifest, remove_old=False, save_path=None, retry_num=10):
    ...
    if os.path.isfile(app_path / 'config.json'):
        with open(app_path / 'config.json') as f:
            ...
            existing_k1_values = set()
            merged_dict_list = []
            for package_dlcs in [config['packagedlcs'], package['packagedlcs']]:
                for d in package_dlcs:
                    if d.get('app_id') not in existing_k1_values:
                        merged_dict_list.append(d)
                        existing_k1_values.add(d['app_id'])
            config['packagedlcs'] = merged_dict_list
...
class MyCDNClient(CDNClient):
    ...
    def load_licenses(self):
        ...
        # 改在初始化时获取app_id_list
        packages = self.steam.get_product_info(packages=packages, timeout=30)['packages']
        for package_id, info in iteritems(packages):
    ...
    def my_get_manifests(self, app_id, app, access_denied_apps, branch='public', password=None, filter_func=None,
                         decrypt=False):
        ...
        for depot_id, depot_info in iteritems(depots):
            ...
            if 'depotfromapp' in depot_info:
                shared_depots.setdefault(int(re.search(r'\d+', depot_info['depotfromapp']).group()), set()).add(
                    depot_id)
                continue
                ...
                with lock:
                    if not app_lock[str(app_id)].get(str(depot_id)):
                        if self.check_manifest_exist(str(depot_id), manifest_gid):
                            ...
                            continue
                        is_denied = False
                        if access_denied_apps:
                            for item in access_denied_apps:
                                if item.get('app_id') == app_id and item.get('depot_id') == depot_id:
                                    update = item['update']
                                    next_time = update + 60 * 60 * 24 * 7
                                    if next_time > time.time():
                                        log.info(
                                            f'The manifest has been disabled: {depot_id}_{manifest_gid}, interval from next attempt to retrieve: {next_time}s!')
                                        is_denied = True
                                        break
                        if is_denied:
                            continue
        ...
        for task in tasks:
            ...
            if isinstance(result, ManifestError):
                log.info(f'{result.eresult}')
                rets['code'] = result.eresult
                rets['depot_id'] = result.depot_id
            else:
        ...
        for app_id, depot_ids in iteritems(shared_depots):
            ...
            try:
                mfs = self.my_get_manifests(app_id, self.get_app_depot_info(app_id), access_denied_apps,
                                            filter_func=nested_ffunc)
                rets['manifests'] += mfs['manifests']
                rets['depots'].update(mfs['depots'])
            except Exception as e:
                log.info(f'{e.eresult}')
        return rets
...
def main(args=None):
    ...
    if result != EResult.OK:
        if args.cli:
            ...
            result = SteamClient.login(args.username, args.password, steam.login_key, args.auth_code,
            ...
            for depot_id, depot in fresh_resp['apps'][app_id]['depots'].items():
                if 'manifests' in depot and 'public' in depot['manifests'] and int(
                    ...
                    if not isinstance(manifest_gid, str):
                        continue
                    result_list.append(
                        gevent.spawn(get_manifest, cdn, app_id, depot_id, app, manifest_gid, args.remove_old))
                    gevent.idle()
    try:
