...
class ManifestAutoUpdate:
    ...
    user_access_denied_apps_path = ROOT / Path('access_denied_apps.json')
    key_path = ROOT / 'KEY'
    ...
    def __init__(self, credential_location=None, level=None, pool_num=None, retry_num=None, update_wait_time=None,
                 key=None, init_only=False, cli=False, app_id_list=None, user_list=None, limit=None):
        if level:
        ...
        self.user_access_denied_apps = MyJson(self.user_access_denied_apps_path)
    ...
    def save_user_access_denied_apps(self):
        self.user_access_denied_apps.dump()

    def save(self):
        self.save_depot_info()
        self.save_user_info()
        self.save_user_access_denied_apps()
    ...
    def get_manifest(self, username, password, sentry_name=None):
        ...
        for app_id in app_id_list:
            ...
            manifests = cdn.my_get_manifests(int(app_id), app, self.user_access_denied_apps[
                username] if username in self.user_access_denied_apps else {})
            if manifests['depots']:
            ...
            if not manifests['manifests']:
                if 'code' in manifests:
                    if manifests['code'] == EResult.AccessDenied:
                        with lock:
                            if username not in self.user_access_denied_apps:
                                # 用户没有拒绝访问的depot_id,进行首次添加。
                                self.user_access_denied_apps[username] = [
                                    {"app_id": app_id, "depot_id": manifests['depot_id'], "update": time.time()}]
                            else:
                                if not any(
                                        item.get('app_id') == app_id and item.get('depot_id') == manifests['depot_id']
                                        for item
                                        in
                                        self.user_access_denied_apps[username]):
                                    # 没有记录拒绝访问的depot_id,进行添加。
                                    self.user_access_denied_apps[username].append(
                                        {"app_id": app_id, "depot_id": manifests['depot_id'], "update": time.time()})
                                else:
                                    # 有拒绝访问的depot_id,进行更新时间。
                                    for item in self.user_access_denied_apps[username]:
                                        if item.get('app_id') == app_id and item.get('depot_id') == manifests[
                                            'depot_id']:
                                            item['update'] = time.time()

                continue
                # 尝试获取dlc或额外内容并添加到配置文件(仅添加拥有的DLC)
            package = {'dlcs': set(), 'packagedlcs': [], 'app_token': 0}
            ...
            all_dlc_list = set(
                map(int, app['extended']['listofdlc'].split(','))) if 'extended' in app and 'listofdlc' in app[
                'extended'] else set()
            # 移除没有的dlc
            dlc_list = set(all_dlc_list) & set(cdn.licensed_app_ids)
            if dlc_list:
                dlc_info = {app_id: app for app_id, app in fresh_resp['apps'].items() if
                            app_id in dlc_list}
                download_dlc = [dlc_id for dlc_id, dlc in dlc_info.items() if 'depots' in dlc]
                no_download_dlc = set(dlc_list) - set(download_dlc)
                package['dlcs'] = no_download_dlc
                package['packagedlcs'] = [{'app_id': dlc_id, 'depots': []} for dlc_id in download_dlc]

                for _app_id, _app in dlc_info.items():
                    if _app_id in [dlc['app_id'] for dlc in package['packagedlcs']]:
                        for _depot_id, _depot in _app['depots'].items():
                            if 'manifests' in _depot and 'public' in _depot['manifests'] and int(
                                    _depot_id) in {*cdn.licensed_depot_ids, *cdn.licensed_app_ids}:
                                for dlc in package['packagedlcs']:
                                    if dlc['app_id'] == _app_id:
                                        dlc['depots'].append(_depot_id)
...
if __name__ == '__main__':
    ...
    ManifestAutoUpdate(args.credential_location, level=args.level, pool_num=args.pool_num, retry_num=args.retry_num,
                       ...
                       cli=args.cli, app_id_list=args.app_id_list, user_list=args.user_list, limit=args.limit).run(
        update=args.update)
    if not args.no_push:
